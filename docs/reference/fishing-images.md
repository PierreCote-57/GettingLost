...
- Lake ID → page mapping is in each lake's JSON at `media/data/destinations/lakes/{name}/{name}.json` → `lakeId` field, where `{name}` is the page's filename base — never a slug, which is derived from it
