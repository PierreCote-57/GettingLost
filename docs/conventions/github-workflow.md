...
- `sync-on-push.yml` — automatic incremental on push to main; falls back to full sync when push includes file deletions or when `local/` changes (sync.js doesn't trust its own incremental logic after a self-edit); triggers on `pages/**`, `posts/**`, `logs/**`, `media/**`, `local/sync/**`
