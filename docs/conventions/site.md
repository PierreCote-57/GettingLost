...
- **After changing destination content, check the cross-references.** Pages point at each other by `file` — a lake's "Destinations" notes section names nearby rec-sites and parks. A rename on one side leaves a link that fetches a 404 at runtime, and nothing at build time catches it. Only rows carrying a `file` link are in scope; a catalog-only row has nothing to check against. Run `python3 local/tools/crossref_check.py` — see [../recipes/crossref-check.md](../recipes/crossref-check.md). A one-way link is correct, so reciprocity is deliberately not checked, and neither is prose agreement between the two sides (both ruled 2026-08-10).
...
- **A site behind a reservation link belongs in the useful links page's `Camping, reservations` group.** When a destination gains a `type: "reservation"` link (or a Further-readings entry that books a site), the booking platform itself goes on `media/data/about/useful-links/useful-links.json` if it is not already there. A destination's OWN booking page is not a separate site — Pacific Playgrounds books on its own domain, so nothing is listed for it. Ruled 2026-08-21; Let's Camp was the case that surfaced it.
