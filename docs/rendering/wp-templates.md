...
```
`gl-constants.jst` MUST stay directly above `gettinglost.jst`. Editing: Site Editor → Patterns → Template Parts → Footer, use LIST VIEW (script-only Custom HTML renders invisibly), or via the site-editing MCP. Per-page scripts (`lakes.jst`, `list_browser.jst`) are inline `<script>` in the page CONTENT, not the footer — which is why `list_browser.jst` runs BEFORE `gl-constants.jst` defines `window.GL`. All `.jst`/`.cst` upload to `wp-content/uploads` via the sync pipeline. See [docs/schema/map-pins-location.md](../schema/map-pins-location.md).

## gl-photo two-up row (how the editor adds top images now)
Two side-by-side images in body content: `<div class="gl-photo" style="width:45%;float:left;" data-img="a.jpg"></div>` + a second with `float:right`, both before the text, then `<div style="clear:both;"></div>` if you don't want following text filling the center gap. gl-photo is float-based; renderer at [docs/rendering/blocks.md](blocks.md) / gettinglost.jst `renderPhoto()`.
