...
- **`location`, a named entry and a pin are one shape** — `location` is the union and the only one that never points, so resolution is one hop. After resolving, a top-level `icon` becomes a `pinList` entry at the centre.
- **A pin** `{icon?, displayName?, img?, url?, lat?, lng?}`: absent `lat`/`lng` → the map's centre. `img` with `/` = `"galleryKey/itemId"` gallery ref (pulls img via `formatImageUrl(img,320,240)` + label); without `/` = direct file. `icon`→custom marker else default; resolved `img`→click InfoWindow (label in `setHeaderContent` — the `.gm-style-iw-ch` header reserves blank space otherwise); label-only→hover `title`; neither→plain marker. label chain: `pin.displayName`→gallery label→the `img` string→none. No automatic page-name label. **`url` makes the marker a link** (a ready href, resolved by whoever builds the pin) — with no `img` the click navigates, with an `img` the InfoWindow opens and its photo and header carry the link.
- **Live center/zoom caption** under EVERY map (no width gate): centered `<div>` wired to the map's `idle` event, `Center = (lat, lng) · Zoom = n`, 6-decimal. Authoring aid to dial in lat/lng/zoom; "public for now, hide later." `location` stays top-level (the list browser's Location column reads it — see [docs/schema/map-pins-location.md](../schema/map-pins-location.md)).
- **The block div is a WRAPPER; the map and the caption are both children of it** (fixed 2026-08-01). The caption used to be a *sibling* inserted `afterend`, faking containment by copying the map's inline `float` and `width` onto itself. That only held while those two properties described the layout — with `float:none` and a pixel width (the `<details>` road maps) the caption went back to the main column at full page width, under nothing. The caption is as wide as the map because it is *in* the map's box.
...
**Map size/float/margin are CSS, and the renderer never touches them (2026-08-21).** `drawMap` reads and writes no box property; it adds `.gl-mapbox` and builds the two children. `gettinglost.cst` carries the rest: `.gl-mapbox` is a flex column with `min-height:400px`, `.gl-map` takes `flex:1`, `.gl-map-caption` its own line — so a block states only its OUTSIDE size and the caption takes its line out of that, instead of being added on underneath. The house style `width:45%; float:right; margin:0 0 1rem 1.5rem` is on the `[data-block-type="googleMap"]` selector, so it reaches page blocks and nothing else — the list browser's map, drawn by the same `drawMap`, inherits none of it and sizes its own container.

Any per-page override is still just inline `style` on the block div, which beats the rule the ordinary way. Don't add `data-width`/`data-height`. Legal `float`: left / right / none. Height now means the BLOCK, caption included, so a `height:500px` map draws about 470 and the caption sits in the remaining line — before this the authored height was moved onto the inner map and the wrapper grew past it.
...
**maintenanceActual** (in `maintenance.jst`, 2026-08-09) — `<div data-block-type="maintenanceActual"></div>` on the two vehicle maintenance pages. Reads `pageData.actual`, a list whose top-level keys ARE the columns in order: `date`, `odometer_km`, `shop{name,url}`, `work{name,url}`, `cost_cad`, `nextDue{km,date}`. `notes` on those pages is the PLAN, `actual` is the RECORD, and the record sits above the plan. `shop.url` links out; `work.url` is the invoice — a bare filename beside the page JSON, uploaded flat to `/wp-content/uploads/`, so invoice filenames must be unique SITEWIDE, not per folder. No url on either = plain text. **A missing value renders blank, never a placeholder** (Pierre, 2026-08-09) — deliberately unlike lakes.jst's `NA`. `nextDue` prints whichever of km/date is known, two lines when both. Absent or empty `actual` renders nothing at all, silently.
...
**Maps are ONE code path (2026-08-21):** `window.GL.drawMap(el, mapObject, galleries?)` is exported from gettinglost.jst and draws every map on the site. The block renderer resolves its named entry and calls it; list_browser.jst builds a mapObject from its rows and calls the same function, so the list map gets the shared pins, icons and centre/zoom caption without a second Google map. `GL.loadGoogleMapsApi` and `GL.pinIcon` were exported for the old separate list map and are internal again. See [docs/schema/map-pins-location.md](../schema/map-pins-location.md).
...
## Authored data table — .gl-table (2026-08-23)
`<table class="gl-table">` for tables written by hand in a page's HTML (not the renderer-built ones above). The CSS lives in gettinglost.cst and carries borders, padding, centered cells and the header band, so the markup holds no presentational attributes and no per-cell `style=`. Column widths come from a `<colgroup>` in the page — `table-layout: fixed` makes them hold regardless of content, and the one `<col>` left without a width takes the remainder. Cells are centered and vertically middled, except the first and last columns, which read left: first = the row label, last = the notes column that absorbs the leftover width. Those two are also the only ones with side padding — on a centered column it never shows, and it steals the width a long header needs (`Generator` at 10% was overflowing its cell). First use: `pages/van/howto/howto-battery.html`.
