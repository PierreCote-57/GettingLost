...
> **PARTLY STALE (2026-07-20 schema unification) — see [docs/projects/schema-unification.md](../projects/schema-unification.md) for current shapes.** Changed since: the `campground` renderer now reads `campground.links` (not `website`/`siteMap`/`reservation`); new shared `linkRow` + `onLostHref` and a new `links` block renderer (HomePage→"Website", OnLost→"On Getting Lost"); the `notes` renderer resolves OnLost scheme-less `*.html` urls; the lake `destinations` renderer + block are RETIRED (folded into a `notes` "Destinations" section). The dispatch/registry/filename-master mechanics below are still accurate.
...
**campground** (CURRENT, 2026-07-21) — the logistics row rendered right **under the page title** (standardized top-of-page slot), one line of EXACTLY three by-label lookups, `· `-separated: **Website · Campground map · Reservation**. Nothing else in the data appears in this row.
- **Website** = the `HomePage` entry of top-level `links[]` (displayed "Website"); the place's official page (BC Parks etc.). Do NOT also duplicate in Further readings.
- **Campground map** = the `campground.links` entry whose label is exactly **`Campground map`** (an external gov PDF URL is fine here — this is where a map belongs, not Further readings). A "park map" and a "campground map" differ: park map = whole-park view (boundary, trails, day-use); campground map = campsite loops/numbered sites for booking. Label follows what the PDF depicts; when one PDF covers both (Elk Falls), label it `Campground map`.
- **Reservation** = the `campground.links` entry whose label is exactly **`Reservation`** — a pre-populated booking URL (camping.bcparks.ca `create-booking/results?resourceLocationId=…`). Trim ephemeral params (transactionLocationId/startDate/endDate/searchTime/flexibleSearch); keep the stable ones. `subEquipmentId=-32765` = Van/Camper.
These two exact labels (`Campground map`, `Reservation`) are the standing convention Pierre set for `campground.links`. Other `campground.links` entries (e.g. a `Reservable; open …` status with `url:null`) are carried but NOT shown in this row. Renderer: `findByLabel()` in gettinglost.jst.
...
## lakes.jst
Lake-specific, loaded via inline `<script src>` (not footer), so it runs before gettinglost.jst's render. Adds the **"destinations"** (renamed from "recSites" 2026-07-18) and "fishingReferences" renderers. Its old private `buildHeading` duplicate was deleted 2026-07-04 — now calls `window.GL.buildGlHeading` (safe: its renderers fire during gettinglost's render pass, after GL is set up). `buildReferencesCell` (destinations + fishing references) supports internal `file` (same-tab, via `window.GL.fileToSlug`) vs external `url` (new tab) — same convention as `notes` (2026-07-08).

**`destinations` renderer (renamed from `recSites` 2026-07-18)** — reads `pageData.destinations = [{name, type, references, description, lat, lng}]` → heading **"Destinations"** + a **Name / Type / References** table (block `<div data-block-type="destinations">`; empty → "No destination"). Type cell = `dest.type || "Rec site"` — a destination is a rec site by default; set `type` to "Park" / "Private" / any label to override. The rename was applied sitewide: JSON key `recSites`→`destinations` in all 10 lake JSONs + the lake template, and the block-type in all lake HTMLs + template. The old **ID (`recId`)**, **Site map (`siteMap`)** and **Reservation (`reservationUrl`/`reservationLabel`)** columns were **removed**, and their helpers `buildLinkOrNaCell` + `buildReservationCell` deleted. Leftover `recId`/`siteMap`/`reservationUrl` may still sit in some lake JSONs as now-ignored data; when a removed field held a useful link (e.g. keogh's letscamp reservation), move it into that lake page's `notes` Further readings. Nearby-place links (park/private/rec-site) live in this table; the campground/reservation *details* for a place live on that place's own page.
