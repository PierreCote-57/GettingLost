...
One file per project, and this index carries its status. **Status lives here and nowhere
else** — a project's own file describes the work as it stands now, with no "phase 3 done",
no "pending", no account of how it got there. That narration is what rots; the file outlives
it.

When a project completes: mark it complete here, then strip the status and history out of
its file, leaving only what is true about the thing that now exists.

| Project | Status | What it is |
| --- | --- | --- |
| [schema-unification.md](schema-unification.md) | **Complete** — 2026-07-20 | One schema and one derivation for destinations. The unified JSON shape, the five principles that still govern the pipeline, and sync-time hydration. |
...
| [destinations-overview.md](destinations-overview.md) | **Live dataset** | The Vancouver Island destinations dataset — campgrounds, lakes, parks, rec sites: place schema, the two row kinds, column-to-field mapping, editing rules, standing data caveats. The page that once rendered it is retired; the data is current. |
